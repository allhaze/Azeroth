...
//        ObjectGuid guid = player->GetGUID();
    ...
    bool OnGossipHello(Player* player, Creature* me)
    {
        ...
        if(player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5)) {
            ...
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "Sign up 1v1 Arena (unrated)", GOSSIP_SENDER_MAIN, 20);

            if(player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_5v5)) == 0)
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, "Create new 1v1 Arenateam", GOSSIP_SENDER_MAIN, 1, "Create 1v1 arenateam?", sWorld->getIntConfig(CONFIG_ARENA_1V1_COSTS), false);
            else
            {
                if(player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5) == false)
                {
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "Sign up 1v1 Arena (rated)", GOSSIP_SENDER_MAIN, 2);
                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, "Disband Arena Team", GOSSIP_SENDER_MAIN, 5, "Leave queue?", 0, false);
                }

                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Show statistics", GOSSIP_SENDER_MAIN, 4);
            }
        }
