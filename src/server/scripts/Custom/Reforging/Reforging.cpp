...
public:
    ...
    {
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Select slot of the item to reforge:", 0, Melt(MAIN_MENU, 0));
            ...
            if (Item* invItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, slot))
                if (IsReforgable(invItem, player))
                    if (const char* slotname = GetSlotName(slot, player->GetSession()))
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, slotname, 0, Melt(SELECT_STAT_REDUCE, slot));
        }
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Remove reforges", 0, Melt(SELECT_RESTORE, 0));
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Update menu", 0, Melt(MAIN_MENU, 0));
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 melt) override
    {
        ...
        case SELECT_STAT_REDUCE:
            ...
            if (Item* invItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, action))
            {
                if (IsReforgable(invItem, player))
                {
                    uint32 guidlow = invItem->GetGUID().GetCounter();
                    const ItemTemplate* pProto = invItem->GetTemplate();
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Stat to decrease:", sender, melt);
                    for (uint32 i = 0; i < pProto->StatsCount; ++i)
                    {
                        ...
                        if (stat_diff > 1)
                            if (const char* stat_name = GetStatName(pProto->ItemStat[i].ItemStatType))
                            {
                                ...
                                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, oss.str(), guidlow, Melt(SELECT_STAT_INCREASE, i));
                            }
                    }
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Back..", 0, Melt(MAIN_MENU, 0));
                    player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
                }
                else
                {
        ...
        case SELECT_STAT_INCREASE:
            ...
            // action = StatsCount id
            {
                ...
                if (invItem)
                {
                    ...
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Stat to increase:", sender, melt);
                    ...
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Back..", 0, Melt(SELECT_STAT_REDUCE, invItem->GetSlot()));
                    player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
                }
                else
                {
        ...
        case SELECT_RESTORE:
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Select slot to remove reforge from:", sender, melt);
                if (!player->reforgeMap.empty())
                {
                    for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
                    {
                ...
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Update menu", sender, melt);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Back..", 0, Melt(MAIN_MENU, 0));
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            }
            break;
